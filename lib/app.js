...
app.audioSupported = function audioSupported(){
...
var User = Backbone.Model.extend({
  initialize      : function (){
    ...
    this.set({
      ...
      "logo"         : chrome.runtime.getURL("icons/default_userpic.png"),
      "name"         : "Twitchuser"
    });
  ...
  populateUserInfo: function (cb){
    ...
    twitchApi.send("user", {}, function (err, res){
      if ( !err ) {
        self.set({
          logo: res.logo || chrome.runtime.getURL("icons/default_userpic.png"),
...
var Control = Backbone.Model.extend({
  ...
  settingsRequirements: {
    sound        : function (){
      return app.audioSupported();
    },
    notifications: function (){
      return app.richNotificationsSupported() || app.htmlNotificationsSupported();
    }
  },
...
//  chrome.browserAction.setIcon({ path: "icons/19_2.png" });
})

twitchApi.on("revoke", function (){
//  chrome.browserAction.setIcon({ path: "icons/19_2_grey.png" });
  following.reset();
  app.clearBadge();
});
