...
app.sendNotification = function ( streamList ) {
    ...
    if ( richNotificationsSupported ) {
        try {
            var message = streamList.splice( 0, 5 ).join( "\n" ) + "\n" + ( streamList.length > 0 ? ( "and " + streamList.length + " more are live now" ) : "are live now" );

            var opt = {
                type   : "basic",
                title  : "Twitch Now",
                message: message,
                iconUrl: "../icons/128_1.png"
            }
    ...
    if ( following.addedStreams.length > 0 ) {
        if ( settings.get( "showDesktopNotification" ).get( "value" ) ) {
            app.sendNotification(
                following.getNewStreams().map( function ( c ) {
